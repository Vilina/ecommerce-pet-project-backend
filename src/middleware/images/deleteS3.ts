...
export const deleteImagesFromBucket = async (images: string[]) => {
    try {
        const results = await Promise.all(images.map(async (key: string) => {
        ...
        const failedDeletes = results.filter(result => result.$metadata.httpStatusCode !== 204);
        if (failedDeletes.length > 0) {
            throw new Error(`Failed to delete some images: ${JSON.stringify(failedDeletes)}`);
        }
