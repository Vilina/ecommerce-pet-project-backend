...
class TokenDao extends BaseDao<IToken> {
    ...
    async blacklistToken(token: string, expiryDate: Date, userId: Types.ObjectId): Promise<void> {
        const existingToken = await this.findOne({ token });
        if(!existingToken) {
            await this.create({ token, expiresAt: expiryDate, type: 'blacklist', userId });
        }
    }
    /**
